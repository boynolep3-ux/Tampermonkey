...
(function () {
    ...
    function kunciLayarBlank() {
        ...
        styleProtector.innerHTML = `
            html, body { background: #ffffff !important; visibility: hidden !important; opacity: 0 !important; overflow: hidden !important; }
        `;
        if (document.documentElement) { document.documentElement.appendChild(styleProtector);
        } else {
            const observer = new MutationObserver(function () {
                if (document.documentElement) { document.documentElement.appendChild(styleProtector); observer.disconnect(); }
            });
    ...
    function bukaKunciLayar() {
        ...
        if (protector) { setTimeout(() => { protector.remove(); }, 50); }
    }
    ...
    function redamErrorJamBawaan() {
        if (isRiwayat || isIframe) {
            if (!document.getElementById('jam')) {
                ...
                fakeClock.id = 'jam'; fakeClock.style.display = 'none';
                if (document.body) document.body.appendChild(fakeClock);
                else if (document.documentElement) document.documentElement.appendChild(fakeClock);
            }
        }
    }
    ...
    if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', mulaiLogikaUtama);
    } else { mulaiLogikaUtama(); }
    ...
    function mulaiLogikaUtama() {
        ...
        if (!document.getElementById('select2-core-css') && (window.location.pathname.includes('/penjualan') || isPelanggan)) {
            ...
            s2Css.id = 'select2-core-css'; s2Css.rel = 'stylesheet'; s2Css.href = 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css';
        ...
        if (!document.getElementById(styleId)) {
            ...
            style.innerHTML = `
                ...
                .xdsoft_datetimepicker:not([style*="display:none"]) .xdsoft_mounthpicker { display: block !important; }
                @keyframes select2FadeSlide { from { opacity: 0; transform: translateY(-8px) scale(0.98); } to { opacity: 1; transform: translateY(0) scale(1); } }
                .select2-container--open .select2-dropdown { animation: select2FadeSlide 0.25s cubic-bezier(0.25, 1, 0.3, 1) forwards; border: 1px solid #ddd !important; border-radius: 8px !important; box-shadow: 0 6px 20px rgba(0,0,0,0.1) !important; overflow: hidden; }
                .select2-container .select2-selection--single { height: 30px !important; border: 1px solid #ccc !important; border-radius: 4px !important; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; font-size: 13px; }
                ...
                .select2-results__option { padding: 6px 10px !important; font-size: 13px !important; }
                .select2-results__option[id*="-Silahkan"], .select2-results__option[id*="-silahkan"] { display: none !important; }
                .panel-body { transition: all 0.8s cubic-bezier(0.25, 1, 0.3, 1); }
                @keyframes melarDanSwipeRight { 0% { opacity: 0; max-height: 0; transform: scale(0.99) translateX(-15px); margin-bottom: 0; padding-top: 0; padding-bottom: 0; } 100% { opacity: 1; max-height: 600px; transform: scale(1) translateX(0); margin-bottom: 15px; } }
                ...
                .modal-content { overflow: hidden !important; border-radius: 12px !important; border: none !important; box-shadow: 0 10px 30px rgba(0,0,0,0.15) !important; transition: max-height 0.65s cubic-bezier(0.25, 1, 0.3, 1) !important; max-height: 140px; }
                .modal-content.modal-expanded { max-height: 2000px !important; }
                #ModalContent, .modal-header, .modal-footer { opacity: 0; transform: translateY(10px); transition: opacity 0.45s cubic-bezier(0.25, 1, 0.3, 1), transform 0.45s cubic-bezier(0.25, 1, 0.3, 1); }
                .modal-expanded #ModalContent, .modal-expanded .modal-header, .modal-expanded .modal-footer { opacity: 1; transform: translateY(0); }
                input[name="uk_panjang[]"]{ min-width: 70px !important; } input[name="uk_lebar[]"]{ min-width: 70px !important; } input[name="jumlah_beli[]"]{ min-width: 70px !important; }
                .input-group-custom { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background:#fff; padding:10px; border:1px solid #ddd; border-radius:5px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); overflow: hidden; box-sizing: border-box; }
                ...
                .row-flex { display: flex; gap: 10px; width: 100%; } .col-flex { flex: 1; display: flex; flex-direction: column; gap: 3px; position: relative; }
        ...
        if (isRiwayat) {
            ...
            // --- INTEGRASI TOMBOL WA NOTIFIER (HANYA DI HALAMAN RIWAYAT UTAMA) ---
            if (!isIframe) {
                ...
                function suntikTombolKeTabel() {
                    ...
                    rows.forEach(row => {
                        ...
                        btnWA.style.cssText = `
                            display: inline-block !important; margin-left: 2px !important; background-color: #25D366 !important; border: 1px solid #ccc !important; padding: 1px 5px !important; height: 22px !important; width: 30px !important; border-radius: 3px !important; cursor: pointer !important; text-align: center !important; box-sizing: border-box !important; vertical-align: middle !important;
                        `;
                        ...
                        btnWA.addEventListener('click', function(e) {
                            ...
                            let cekPopUp = setInterval(function() {
                                ...
                                if (textareaUtuh && textareaUtuh.value.trim() !== "") {
                                    ...
                                    if (matchNomor) { nomorClean = matchNomor[0].replace(/[^0-9]/g, ''); }
                                    ...
                                    if (nomorClean && nomorClean.length >= 10) {
                                        ...
                                        if (ekstrakNama && ekstrakNama[1].trim() !== "") { namaKonsumenAsli = ekstrakNama[1].trim(); }
                                        ...
                                        GM_setValue('klik_wa_data', {
                                            phone: nomorClean, text: pesanTemplate, timestamp: new Date().getTime()
                                        });
                                    ...
                                    const tombolCloseModal = document.querySelector('.modal-header .close') || document.querySelector('[data-dismiss="modal"]') || document.querySelector('.modal-footer .btn-danger');
                ...
                // SOLUSI ANTI GAGAL: Scan tabel setiap detik agar tombol PASTI nempel meskipun loading web telat
                setInterval(suntikTombolKeTabel, 1000);
            }
        ...
        function suntikIframeMataMata() {
            if ($('#iframe-riwayat-bintang').length === 0) {
                ...
                iframe.id = 'iframe-riwayat-bintang'; iframe.src = 'https://bintangbaru.bintangmedi4.com/datapenjualan/riwayat';
                iframe.style.width = '0px'; iframe.style.height = '0px'; iframe.style.border = 'none'; iframe.style.position = 'absolute'; iframe.style.visibility = 'hidden';
        ...
        function prosesSyncNomorNota(nomorTerakhir) {
            ...
            if (inputNota.length && nomorTerakhir) {
                if (!$('#log-nota-update-time').length) { inputNota.after('<span id="log-nota-update-time" class="log-nota-time animasi-fade-lembut">Menghitung waktu update...</span>'); }
                const m = nomorTerakhir.match(/^([A-Za-z]+)(\d+)$/);
                if (m) {
                    const prefix = m[1]; const numStr = m[2];
                    const nextNum = (parseInt(numStr, 10) + 1).toString().padStart(numStr.length, '0');
                    const notaTargetOtomatis = prefix + nextNum;
                    if (inputNota.val() !== notaTargetOtomatis && !inputNota.is(':focus')) { inputNota.val(notaTargetOtomatis).trigger('change'); }
                }
            }
        }
        ...
        function jalankanSelect2Liquid() {
            if (window.$ && $.fn.select2) {
                $('select:not(.select2-hidden-accessible), #id_pelanggan:not(.select2-hidden-accessible)').each(function() {
                    const self = $(this);
                    self.select2({ width: '100%', dropdownParent: self.parent(), matcher: customSelect2Matcher });
                });
            }
        }
            ...
            window.addEventListener('focus', async () => {
                try {
                    ...
                    if (regexHP.test(text.replace(/[- ]/g, '')) && text !== sessionStorage.getItem('last_pasted_hp')) {
                        ...
                        let currentTxt = $('#catatan').val() || '';
                        const hpMatch = currentTxt.match(/\((?:\+62|62|0)8[1-9][0-9- ]{6,15}\)/);
                        if (hpMatch) { currentTxt = currentTxt.replace(hpMatch[0], `(${text})`); } else { currentTxt = currentTxt + ` (${text})`; }
        ...
        function buatKolomKeteranganBaru() {
            ...
            const container = `
                <div class="input-group-custom animasi-muncul-lembut" style="margin-bottom: 8px;">
                    <div class="row-flex">
                        <div class="col-flex">
                            <label>DP</label> <input type="text" id="input-dp-custom" class="datepicker-custom" readonly placeholder="Tgl DP">
                            <button type="button" class="btn btn-danger btn-block btn-sm btn-clear-date" data-target="#input-dp-custom" title="Hapus DP"> <i class="fa fa-trash fa-lg"></i> </button>
                        </div>
                        <div class="col-flex">
                            <label>TF</label> <input type="text" id="input-tf-custom" class="datepicker-custom" readonly placeholder="Tgl TF">
                            <button type="button" class="btn btn-danger btn-block btn-sm btn-clear-date" data-target="#input-tf-custom" title="Hapus TF"> <i class="fa fa-trash fa-lg"></i> </button>
                        </div>
                        <div class="col-flex">
                            <label>CASH</label> <input type="text" id="input-cash-custom" class="datepicker-custom" readonly placeholder="Tgl CASH">
                            <button type="button" class="btn btn-danger btn-block btn-sm btn-clear-date" data-target="#input-cash-custom" title="Hapus CASH"> <i class="fa fa-trash fa-lg"></i> </button>
                        </div>
                    </div>
                </div>`;
            ...
            if (window.$ && $.fn.datetimepicker) {
                $('.datepicker-custom').datetimepicker({ timepicker: false, format: 'd.m.Y', scrollMonth: false, scrollInput: false, onSelectDate: function(ct, $i) { sinkronkanCatatan(); $i.datetimepicker('hide'); } });
            }
            $('.btn-clear-date').on('click', function() { const target = $(this).data('target'); $(target).val('').trigger('change'); });
        ...
        function HitungUlangBarisRealtime(idx) {
            ...
            const sub = (p * l) * h;
            row.find('td:nth-child(7) input').val(sub); row.find('td:nth-child(7) span').html(sub > 0 ? to_rupiah(sub) : '');
            const tot = sub * j;
            row.find('td:nth-child(8) input').val(tot); row.find('td:nth-child(8) span').html(tot > 0 ? to_rupiah(tot) : '');
            HitungTotalBayar();
        }
        ...
        function inisialisasi() {
            ...
            if (window.location.pathname.includes('/penjualan')) {
                ...
                if (!$('#input-dp-custom').length) {
                    buatKolomKeteranganBaru(); aktifkanAutoClipboard();
                    if (!sudahAutoSetPelanggan && $('#id_pelanggan').length && $('#id_pelanggan').val() === '') {
                        $('#id_pelanggan').val('1').trigger('change'); sudahAutoSetPelanggan = true;
                    }
                }
            }
            bukaKunciLayar();
        ...
        $(document).on('input keyup change', '#ukuran_panjang, #ukuran_lebar', function () { HitungUlangBarisRealtime($(this).closest('tr').index()); });
    ...
    /* =====================================================
       ...
       ===================================================== */
    (function(){
        let keywordTerakhir = ''; let inputAktif = null;
        $(document).on('focus', 'input[name="kode_barang[]"]', function(){ inputAktif = this; keywordTerakhir = ''; });

        setInterval(function(){
            if (!inputAktif) return;
            const $td = $(inputAktif).closest('td'); const $hasil = $td.find('#hasil_pencarian'); const $daftar = $hasil.find('#daftar-autocomplete');
            ...
            setTimeout(function(){
                const daftar = $daftar.find('li'); if (!daftar.length) return;
                const kataCari = keyword.split(/\s+/).filter(Boolean); const hasil = [];
                daftar.each(function(){
                    ...
                    const cocok = kataCari.every(function(k){ return kode.includes(k) || barang.includes(k) || harga.includes(k); });
                    if (cocok) { hasil.push($(this).prop('outerHTML')); }
                });
                if (hasil.length) { $daftar.html(hasil.join('')); $hasil.show(); }
            }, 150);
        }, 200);
    })();

    /* FIX MULTI BARIS */
    $(document).on('focus', 'input[name="kode_barang[]"]', function(){ window.__aktif_kode_barang = this; });
    ...
    (function(){
        ...
        $(document).on('click', '#daftar-autocomplete li', function(){
            ...
            setTimeout(function(){ $hasil.hide(); }, 50); setTimeout(function(){ $hasil.hide(); }, 300); setTimeout(function(){ $hasil.hide(); }, 800);
        });
        setInterval(function(){
            if (!waktuKlikAutocomplete) return;
            if (Date.now() - waktuKlikAutocomplete < 1200) { $('#hasil_pencarian').hide(); }
        }, 100);
    })();
